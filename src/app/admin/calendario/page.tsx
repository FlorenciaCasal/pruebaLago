...
"use client";
import React from "react";
import { getCalendarState, upsertDayCapacity } from "@/services/admin";
import type { CalendarMonthState } from "@/types/admin";
...
export default function CalendarioAdminPage() {
  ...
  const load = React.useCallback(async (yy = y, mm = m) => {
    setLoading(true);
    try {
      const s = await getCalendarState(yy, mm);
      setState(s);
  ...
  // Alterna un día: capacity 0 ↔ DEFAULT_CAPACITY
  const toggleDay = async (dateISO: string) => {
    if (!state) return;
    setBusy(true);
    const isDisabled = state.disabledDays.includes(dateISO);
    try {
      await upsertDayCapacity(dateISO, isDisabled ? DEFAULT_CAPACITY : 0);
      setState(prev =>
        prev ? {
          ...prev,
          disabledDays: isDisabled
  ...
  // Alterna el mes completo (batch sobre cada día)
  const toggleMonth = async () => {
    if (!state) return;
    setBusy(true);
    try {
      const dates = allDateISOsOfMonth(state.year, state.month);
      const make = async (dateISO: string, cap: number) => {
        await upsertDayCapacity(dateISO, cap);
      };
      const targetDisabled = !state.disabled; // si estaba habilitado -> bloquear todo (cap=0)
      const cap = targetDisabled ? 0 : DEFAULT_CAPACITY;

      // Ejecutamos en tandas para no saturar
      const CHUNK = 5;
      for (let i = 0; i < dates.length; i += CHUNK) {
        await Promise.all(dates.slice(i, i + CHUNK).map(d => make(d, cap)));
      }
      ...
      setState(prev => prev
        ? {
            ...
            disabledDays: targetDisabled ? dates : [],
          }
        : prev
      );
    } catch (e) {
  ...
  // Grilla de mes
  const buildCells = React.useMemo(() => {
    const first = new Date(y, m - 1, 1);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => { const d = new Date(y, m - 2, 1); setYM(ym(d)); void load(d.getFullYear(), d.getMonth() + 1); }}
            className="rounded-lg border border-neutral-700 px-3 py-1.5 hover:bg-neutral-800"
          >◀</button>
          <div className="rounded-lg border border-neutral-700 px-3 py-1.5 bg-neutral-950 capitalize">
            {monthLabel}
          </div>
          <button
            onClick={() => { const d = new Date(y, m, 1); setYM(ym(d)); void load(d.getFullYear(), d.getMonth() + 1); }}
            className="rounded-lg border border-neutral-700 px-3 py-1.5 hover:bg-neutral-800"
          >▶</button>
        </div>
      </div>
      ...
      {loading || !state ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-neutral-400">Hacé clic en un día para alternar su estado.</span>
          </div>

          <div className="grid grid-cols-7 gap-2 rounded-2xl border border-neutral-800 p-4 bg-neutral-950">
            {["Do","Lu","Ma","Mi","Ju","Vi","Sa"].map(h => (
              <div key={h} className="text-center text-neutral-400 text-xs">{h}</div>
            ))}
            {buildCells.map((d, i) => {
              if (!d) return <div key={i} />;
              const dateISO = d.toISOString().slice(0, 10);
