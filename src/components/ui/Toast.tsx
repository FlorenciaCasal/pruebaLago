        ...
        <ToastCtx.Provider value={push}>
            ...
            <div className="fixed inset-x-0 top-1/3 -translate-y-1/2 lg:top-7 z-[9999] flex justify-center pointer-events-none">
                ...
                <div className="w-full max-w-md px-3 space-y-2">
                    {toasts.map(t => (
                        <div key={t.id}
                            className="pointer-events-auto rounded-lg border border-white/10 bg-white/10 backdrop-blur px-4 py-2 text-white shadow-lg animate-in fade-in slide-in-from-top-2">
                            {t.msg}
                        </div>
                    ))}
                </div>
